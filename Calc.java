...
public class Calc {
    ...
    public static void main(String[] args) {
      ...
      String[] parts = new String[2]; // contains the 2 numbers to be calculated
      char operator = ' ';
      if(operation.contains("+")){ parts = operation.split("\\+");operator = '+'; }
      else if(operation.contains("-")){parts = operation.split("-");operator = '-';}
      else if(operation.contains("*")){parts = operation.split("\\*");operator = '*';}
      else if(operation.contains("/")){parts = operation.split("/");operator = '/';}
      else {System.out.println("invalid input");} // fill parts array, assign operator var
      
      //parsing the numbers from parts to calculate them
      double result = 0;
